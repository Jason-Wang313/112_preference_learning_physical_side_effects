        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Preference Learning Physical Side Effects: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study human preference learning for robots. The submission-hardened claim is intentionally narrow: Learn preferences over physical side effects that are absent from task success labels. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Preference learning physical side effects should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.408 $\pm$ 0.020 & 0.450 $\pm$ 0.015 & 0.330 $\pm$ 0.005 \\
Data-augmented WAM & 0.493 $\pm$ 0.015 & 0.514 $\pm$ 0.017 & 0.425 $\pm$ 0.021 \\
Scalar uncertainty & 0.553 $\pm$ 0.012 & 0.591 $\pm$ 0.014 & 0.493 $\pm$ 0.019 \\
Ensemble disagreement & 0.604 $\pm$ 0.010 & 0.618 $\pm$ 0.010 & 0.554 $\pm$ 0.006 \\
Conformal risk filter & 0.614 $\pm$ 0.007 & 0.619 $\pm$ 0.023 & 0.571 $\pm$ 0.018 \\
Proposed branch mechanism & 0.700 $\pm$ 0.012 & 0.701 $\pm$ 0.014 & 0.653 $\pm$ 0.015 \\
Oracle branch upper bound & 0.764 $\pm$ 0.010 & 0.771 $\pm$ 0.011 & 0.734 $\pm$ 0.014 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.659 $\pm$ 0.024 & all components \\
Minus branch atlas & 0.566 $\pm$ 0.014 & removes explicit unrealized futures \\
Minus tail risk & 0.584 $\pm$ 0.012 & uses mean predicted value only \\
Minus diagnostic probe & 0.605 $\pm$ 0.011 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.607 $\pm$ 0.014 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/112.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Preference Learning Physical Side Effects is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
